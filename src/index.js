...
const mongodbUrl = process.env.REEL_REVIEWS_MONGODB_URL || "mongodb://localhost/reelreviews";

Mongoose.connect(mongodbUrl, { useNewUrlParser: true }).catch(error => { console.log(error); });
Mongoose.connection.on('error', error => console.log(error));
Mongoose.connection.on('connected', () => console.log(`Mongoose connected to ${mongodbUrl}`));
Mongoose.connection.on('disconnect', () => console.log(`Mongoose has disconnected from ${mongodbUrl}`));

process.on('SIGINT', () => {
  Mongoose.connection.close(() => {
    console.log(`Mongoose disconnection to ${mongodbUrl} occurred due to application termination.`);
    process.exit(0);
  });
});
...
app.delete("/favorite/:id", async (request, response) => {
  try {
    let favoriteModel = await FavoriteModel.deleteOne({ _id: request.params.id }).exec();
    response.send(favoriteModel);
  } catch (error) {
    consople.log(error);
    response.status(500).send(error);
